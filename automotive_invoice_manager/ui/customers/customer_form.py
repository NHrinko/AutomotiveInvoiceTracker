# automotive_invoice_manager/ui/customers/customer_form.py - ENHANCED VERSION
...
from automotive_invoice_manager.ui.theme import FONTS, COLORS

logger = logging.getLogger(__name__)


class CustomerFormDialog:
    """Enhanced dialog for creating and editing customers."""

    def __init__(self, parent, customer_service, user, customer=None):
        ...
        Args:
            ...
            customer: Customer to edit (None for new customer)
        """
        self.parent = parent
    ...
    def create_form(self):
        ...
        # Name field (required)
        self.create_field(form_frame, 0, "Customer Name", self.name_var, required=True)
        ...
        self.create_text_field(form_frame, 3, "Address", 3)

        # Notes field (multiline)
        self.create_text_field(form_frame, 4, "Notes", 3)
    ...
    def create_field(self, parent, row, label_text, var, required=False):
        ...
        entry = ttk.Entry(entry_frame, textvariable=var, font=FONTS['base'], width=40)
        ...
        # Store references - FIXED naming
        if "Customer Name" in label_text:
            setattr(self, "name_entry", entry)
            setattr(self, "name_error", error_label)
        elif "Email" in label_text:
            setattr(self, "email_entry", entry)
            setattr(self, "email_error", error_label)
        elif "Phone" in label_text:
            setattr(self, "phone_entry", entry)
            setattr(self, "phone_error", error_label)
        else:
    ...
    def create_text_field(self, parent, row, label_text, height):
        ...
        ttk.Label(parent, text=f"{label_text}:").grid(
            row=row, column=0, sticky=tk.NW, padx=(0, 10), pady=8
        )
        ...
        try:
            ...
            if self.customer:
                ...
                success = self.customer_service.update_customer(
                    self.customer.id, customer_data, self.user.id
                )
                action = "updated"
            else:
                # Create new customer
                customer = self.customer_service.create_customer(
                    customer_data, self.user.id
                )
...
# Simple customer creation function for quick access
def create_customer_simple(parent, customer_service, user, name=None):
    """Simple function to create a customer with minimal info."""
    try:
        if not name:
        ...
        customer = customer_service.create_customer(customer_data, user.id)
